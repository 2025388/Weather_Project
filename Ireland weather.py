...
def process_single_day(date_tuple):
    ...
    # Define log path
    log_path = r"C:\Users\dmoli\Documents\Coding\ME Project\date_log_ir.csv"
    ...
    'std': 'std',
    'median': 'median',
    'range': lambda x: x.max() - x.min() # Using a named lambda for clarity
}
    ...
    log_path = r"C:\Users\dmoli\Documents\Coding\ME Project\storage_log_ir.csv"
...
def get_all_dates(start_date="1940-01-02", end_date="2025-06-17"):
    if end_date is None:
...
if __name__ == "__main__":
    ...
    # Load skip list from dates.csv
    df_dates = pd.read_csv(r"C:\Users\dmoli\Documents\Coding\ME Project\sample2.csv")
    dates_to_skip = [(int(row['year']), int(row['month']), int(row['day'])) for index, row in df_dates.iterrows()]

    # Remove skip dates from full list
    all_dates = [d for d in all_dates if d not in dates_to_skip]
